#!/usr/bin/env python3
"""Discover topics using Stochastic Block Model (Spectral Co-clustering).

The SBM approach builds a word-document bipartite graph and infers block
structure, simultaneously clustering documents and words. Each block is a topic.

Usage:
    python3 scripts/topics/04_sbm_topics.py --version-id <uuid>

Create a version first:
    python3 -c "
    from src.versions import create_version, get_default_sbm_config
    vid = create_version('sbm-baseline', 'SBM topic analysis', get_default_sbm_config(), 'topics')
...
from src.topics_sbm import discover_sbm_topics
from src.versions import get_version, get_version_config, update_pipeline_status


def main():
    parser = argparse.ArgumentParser(description="Discover topics using SBM (Spectral Co-clustering)")
    ...
    print("=" * 60)
    print("Topic Discovery with Stochastic Block Model")
    print("=" * 60)
    ...
    summary = discover_sbm_topics(
        result_version_id=args.version_id,
        topic_config=topic_config,
    )

    print("\n" + "=" * 60)
    print("Discovered Topics (Blocks):")
    print("=" * 60)

#!/usr/bin/env python3
"""
Generate Ditwah Claims - Batched Approach

Efficient approach:
- Process articles in batches of 50
- Extract claims with article mappings
- Deduplicate similar claims
- Ensure all claims have >=5 articles
- Ensure all claims have both sentiment AND stance data

Usage:
    python3 scripts/ditwah_claims/04_generate_claims_batched.py --version-id <uuid>

Example:
    python3 scripts/ditwah_claims/04_generate_claims_batched.py --version-id 123e4567-e89b-12d3-a456-426614174000
"""
...
from src.ditwah_claims_batched import generate_claims_pipeline_batched
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate claims from Ditwah articles (batched approach)"
    )
    parser.add_argument(
        "--version-id",
        required=True,
    ...
    logger.info("  1. Process articles in batches of 50")
    logger.info("  2. Generate claims from each batch")
    logger.info("  3. Merge and deduplicate claims")
    logger.info("  4. Filter claims with < 5 articles")
    logger.info("  5. Analyze sentiment and stance for all claims")
    logger.info("  6. Keep only claims with BOTH sentiment and stance data\n")
    ...
    try:
        ...
        logger.info(f"After merging duplicates: {summary['merged_claims']}")
        logger.info(f"After LLM deduplication: {summary['deduplicated_claims']}")
